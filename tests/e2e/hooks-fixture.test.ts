...
describe("e2e: plugin/hooks/on-edit.sh", () => {
  ...
  it("does nothing for a non-*.tldx.jsx file path", () => {
    const result = runHook(HOOK, { tool_input: { file_path: "/tmp/x.ts" } }, { TLDX_BIN });
    expect(result.stdout).toBe("");
    expect(result.status).toBe(0);
  }, 30_000);

  it("does nothing for a *.tldx.jsx path that does not exist", () => {
  ...
  it("surfaces check diagnostics for a broken fixture", () => {
  ...
  it("reports clean with no warm serve for a clean fixture", () => {
...
describe("e2e: plugin/hooks/on-prompt.sh", () => {
  ...
  it("reports unabsorbed canvas changes", () => {
  ...
  it("stays silent for an overlay with no entries", () => {
    writeFileSync(join(dir, "a.tldx.jsx"), "export default function Diagram() {}\n");
    writeFileSync(
      join(dir, "a.tldx.overlay.json"),
