...
- [x] **T39. Re-test the skill on unseen material.**
  ...
  **The first attempt failed `check`, on one word.** A fresh sonnet agent given
  the path to that one file, forbidden from reading anything else or running any
  CLI, and allowed a single write, produced a clean seven-tier `<Layers>` stack
  with a `flow()` chain down it - and `check` rejected it at line 6:
  `error[ir/missing-id]: '<frame>' is addressable and requires an explicit 'id'`.
  Adding `id="osi"` to the `<Layers>`, one attribute and nothing else, makes it
  compile and render legibly. Probing all eight aliases shows the rule is
  universal - `<Row> <Col> <Grid> <Group> <Pipeline> <Layers> <Swimlanes>
  <Graph>` each reject a missing `id` with the identical message - while the
  skill's component table carried required props on `<Frame>` and `<Box>` only,
  so an author reading it has no reason to think an unnamed layout wrapper needs
  one. That is D12 seen at full width; logged as **Also seen** under D12 rather
  than as a new entry, since it is the same mechanism the same way D2's
  `<Graph>` note is.

  **The fix went to the skill**, per the task: a sixth non-negotiable rule
  naming all eight aliases and warning that the diagnostic says `<frame>`
  whatever you wrote, `id` added to every container row of the table, and the
  `ir/missing-id` line under *What `check` will reject* widened from
  "`<Box>` and `<Frame>`" to "every `<Box>` and every container". Nothing in
  `src/` was touched - the diagnostic still names `<frame>` and D12 stays open
  for it.

  **Re-tested blind on the fixed skill and it passes.** A second fresh agent,
  same one sentence, same restrictions, wrote `<Layers id="osi">` with the seven
  tiers generated by `.map()` (no `key`, correctly) and `check` exits 0 on the
  first attempt. Looked at both PNGs: both read cleanly top to bottom, seven
  captioned tiers each holding its protocol, no overlaps and nothing clipped.
  Two blemishes, both already in the ledger and neither fixed here: the
  `<Layers>` wrapper draws a border captioned with the literal word `Frame`
  (D2 - and `<Layers>` has no chrome-free alternative the way `<Row>`/`<Col>`
  have `<Group>`), and the Data Link tier is visibly wider than its six siblings
...
- **T39: the two blind agents disagreed about whether the stack has arrows.**
  ...
  seven tiers and the second drew no edges at all. Both are defensible readings
  of "the OSI model" and both render legibly, but the skill says nothing about
  when a container's own stacking order already carries the reading and an edge
  chain would be redundant. `<Pipeline>` wires its children automatically and
  `<Layers>` does not; that asymmetry is undocumented and is probably what the
  two agents were each guessing at.
- **T39: an edge pointed at a `<Frame>` id works and is undocumented.** The
  first agent's `flow()` chained frame ids, not box ids, and it compiled and
  rendered arrows between the tier borders. The skill only forbids pointing an
  edge at a `<Group>`; it never says a `<Frame>` is a legal endpoint, so an
  author has to guess. Worth one clause in the Edges section.
