...
## 22. Round-trip is `apply(overlay, compile(jsx))`, not a canvas-to-source sync

**Decision**: canvas edits land in `x.tldsl.overlay.json`, a final-state map keyed by tldraw record id, and the render is `applyOverlay(overlay, compile(jsx))` - pure, total, deterministic, and with no model in it. `applyOverlay` lives in `domain/overlay/`, never re-runs layout, and never refuses to run: an entry that no longer resolves becomes a warning, not an error. The viewer PUTs its whole document snapshot to `/overlay`; the server diffs it against the last compiled scene (`diffScenes`) and writes the overlay. Turning overlay entries back into JSX is a separate, human-invoked step (`absorb`, T22). As shipped it is mechanical rather than model-driven: it rewrites source only for the ops JSX can express exactly, leaves the rest in the overlay, and refuses unless the rewrite verifiably reproduces the render - so it is not "imperfect because reviewed", it is narrow and self-checking. A model restructuring the result into frames and components sits on top of it, not inside it.

**Why**: editing a program by editing its output is undecidable for a compiler - a shape emitted from a `.map()` has no unique source edit. Quarantining the part that needs intent makes the lossless part mechanical: the canvas state *is* `compile + overlay`, so a reload recomputes the same function over the same inputs and reproduces exactly. Fidelity stops being an algorithm to get right and becomes an invariant. The design and its open details are settled in `docs/round-trip.md` (D1-D5); that doc refers to this entry as "ADR-14", which was already taken by "JSX as syntax" - it is this one, 22.

**Rejected**: a websocket for the write path (a `PUT` on the existing dev server carries it, and SSE stays one-way - see ADR-13's transport note); watching the overlay file from `serve` (the viewer's own write would push a fresh scene back and stomp the canvas being edited); and re-running layout under an overlay (the overlay would become an input to the thing it patches, and the fixed point is not guaranteed to exist).
