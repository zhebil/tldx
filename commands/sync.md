---
description: >-
  Fold a diagram's canvas edits back into its `.tldsl.jsx` source, then verify
  the source alone reproduces what the canvas showed. Use after moving,
  restyling or adding shapes in the `tldsl serve` viewer.
argument-hint: <file.tldsl.jsx>
allowed-tools: Bash(tldsl:*), Bash(node:*), Bash(git diff:*), Bash(git status:*), Bash(rm:*), Read, Edit
---

# Sync a diagram's canvas edits into its source

The diagram is `$1`. If that is empty, use the `.tldsl.jsx` file this session
has been working on; if there is more than one candidate, ask which.

Everything below is a `tldsl` CLI call. `tldsl` means the binary on `PATH`; if it
is not installed, use `node ${CLAUDE_PLUGIN_ROOT}/dist/cli/main.js` instead.

## 1. Look at what is pending

```
tldsl overlay show $1
```
...
Absorb leaves every entry it cannot generate JSX for. Run `tldsl overlay show $1`
...
`tldsl verify $1` must exit 0. When it passes while the overlay still has
...
`tldsl verify $1` once more to confirm.
...
- **Never parse the overlay JSON yourself.** To learn what is pending, ask
  `tldsl overlay show`. To learn whether the source reproduces the canvas, ask
  `tldsl verify`. If you find yourself wanting to compute something about the
...
- **Never empty or delete the overlay before `tldsl verify` passes.** That check
