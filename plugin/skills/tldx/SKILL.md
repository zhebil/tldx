...
name: tldx
description: >-
  Write architecture and flow diagrams as JSX with tldx, which compiles
  `.tldx.jsx` to a laid-out tldraw canvas. Use whenever a diagram is asked for
  ...
  named `*.tldx.jsx` is being read or edited. Covers the component vocabulary,
  the styling props, and the `check` / `serve` / `render` workflow.
---

# tldx

A diagram is a `.tldx.jsx` file. You write JSX; tldx sizes the boxes, lays out
...
```jsx
import { Doc, Frame, Box, Edges, Sticky } from "tldx";
...
- `"tldx"` resolves without installing anything - the CLI aliases it.
...
Split a diagram across files with plain ES `import` - nothing tldx-specific,
...
Only the entry file needs the `.tldx.jsx` extension - modules it imports are
plain `.jsx`.

```jsx
// diagrams/checkout.jsx
import { Frame, Box, Edge } from "tldx";
