...
import { runExecuteContract, type ExecuteHarness } from "../../app/ports/execute.contract.js";
...
describe("createJsxExecute: adapter-specific behavior", () => {
  it("inputs includes transitively-imported files", async () => {
...
`;
  ...
  it("runtime/compile diagnostic's span points at the entry file", async () => {
  ...
  it("accepts a relative entry path and reports absolute spans", async () => {
  ...
  it("runtime/threw diagnostic's span.line is the actual throw line", async () => {
  ...
  it("runtime/threw diagnostic from an imported file names that file, and inputs covers the module graph", async () => {
